...
import vite, { mergeConfig } from 'vite';
        ...
        async config(config, configEnv) {
            ...
            const serverConfig = mergeConfig(
                config,
                {
                    ...
                    logLevel: 'error',
                    build: {
                        ssr: '/lib/vite-plugin-dlh/entry-server.ts',
                    },
                    server: {
                        hmr: { overlay: false },
                        middlewareMode: 'ssr'
            ...
            server = await vite.createServer(serverConfig);
            
            // server.middlewares;
            
            const clientConfig = mergeConfig(config, {
                build: {
                    ssrManifest: true
                }
            })
